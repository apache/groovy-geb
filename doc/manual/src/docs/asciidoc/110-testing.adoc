...
[WARNING]
====
On a static nested class Groovy <6 inserts synthetic variants of `propertyMissing` and `methodMissing` which will overwrite the versions in the superclasses and not forward to them. (See https://issues.apache.org/jira/browse/GROOVY-11823[GROOVY-11823])
...
This was fixed in Groovy from 6.0 on, so starting with Groovy 6 the dispatching should work again.
====
